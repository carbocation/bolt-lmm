...
#pragma omp parallel for
  ...
  string Bolt::getSnpStatsBgen2(uint CompressedSNPBlocks, uchar *buf, uint bufLen,
    ...
    if (dosageSum < bgenMinMAC || 2*dosageNum-dosageSum < bgenMinMAC) return "";
	...
	for (uint i = 0; i < N; i++) {
	  uint ploidy = ploidyMissBytes[i] & ~missBit;
	  if (Phased==1U) {
	    cerr << "ERROR: --domRecHetTest not implemented for phased BGEN data" << endl;
	    exit(1);
	  }
	  double p11 = lut[*bufAt]; bufAt++;
	  double p10=0; if (ploidy==2U) { p10=lut[*bufAt]; bufAt++; }
	  if (!(ploidyMissBytes[i] & missBit) && bgenIndivInds[i] != SnpData::IND_MISSING) {
	    if (ac == 0)
	      snpCovCompVec[bgenIndivInds[i]] = //2 * (alleleCounts[i] != ac);
		2 * (p11+p10);
	    else if (ac == 1)
  ...
  (const string &outFile, int f, const string &bgenFile, const string &sampleFile,
   ...
   bool bgenRefFirst, int threads) const {
	...
	allele1s[B] = string(allele0);
	allele0s[B] = string(allele1);
      }
      else {
	...
	variant = (char *) malloc(variantLen);
      ...
      sprintf(variant, "%s:%d:%s:%s", chrStr, physpos, allele1, allele0);
