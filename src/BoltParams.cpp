...
namespace LMM {
  ...
  std::unordered_set <string> readBgenVariantsToTest(const string &bgenVariantsToTestFile) {
    cout << "Reading list of bgen variants to test: " << bgenVariantsToTestFile << endl;
    FileUtils::AutoGzIfstream fin; fin.openOrExit(bgenVariantsToTestFile);
    std::unordered_set <string> bgenVariantsToTest;
    string chr, pos, ref, alt;
    string line; int lineCtr = 0;
    while (getline(fin, line)) {
      lineCtr++;
      std::istringstream iss(line);
      if (!(iss >> chr >> pos >> ref >> alt)) {
	cerr << "ERROR: Unable to read four tokens (CHR POS A B) at line " << lineCtr << ":"
	     << endl;
	cerr << line << endl;
	exit(1);
      }
      int i;
      if (sscanf(pos.c_str(), "%d", &i) == 0) {
	cerr << "ERROR: Unable to parse POS into integer at line " << lineCtr << ":"
	     << endl;
	cerr << line << endl;
	exit(1);
      }
      bgenVariantsToTest.insert(chr+":"+pos+":"+ref+":"+alt);
    }
    fin.close();
    cout << "Read " << bgenVariantsToTest.size() << " variants to extract" << endl << endl;
    return bgenVariantsToTest;
  }
  ...
  bool BoltParams::processCommandLineArgs(int argc, char *argv[]) {
    ...
    string bgenVariantsToTestFile;
    uint maxBgenVariantsToScan;
    ...
    typical.add_options()
      ...
      ("bgenMinMAC", po::value<int>(&bgenMinMAC)->default_value(1),
       "minimum MAC threshold (in samples included in association tests) on BGEN v1.2+ genotypes")
      ("bgenVariantsToTest", po::value<string>(&bgenVariantsToTestFile),
       "list of bgen variants to test (CHR POS REF ALT)")
      ("bgenRefFirst", "set effect allele (ALLELE1) to second allele in BGEN v1.2+ genotype file")
    ...
    additional.add_options()
      ...
      ("maxBgenVariantsToScan", po::value<uint>(&maxBgenVariantsToScan)->default_value(100000),
       "an error-check: if --bgenVariantsToTest is supplied, each bgen file is scanned for the presence of a listed variant within the first this many variants")
	...
	return false;
      ...
      if (statsFileBgenSnps.empty() && !bgenVariantsToTestFile.empty()) {
	cerr << "ERROR: --statsFileBgenSnps is required if --bgenVariantsToTest is specified" << endl;
	return false;
      }
      if (!statsFileBgenSnps.empty()) {
	...
	// read bgenVariantsToTest if provided
	if (!bgenVariantsToTestFile.empty())
	...
	for (uint64 f = 0; f < bgenFiles.size(); f++) {
	  ...
	  bgenLayouts[f] = FileUtils::checkBgenSample(bgenFiles[f], sampleFile, Nautosomes,
						      bgenVariantsToTest, maxBgenVariantsToScan);
	}
      }
