...
namespace FileUtils {
  ...
  int checkBgenSample(const std::string &bgenFile, const std::string &sampleFile, int Nautosomes,
    ...
    uchar *zBuf = NULL; uint zBufMaxLen = 0;
    char *variant = NULL; uint variantLen = 0;

    ushort LS; fread_check(&LS, 2, 1, fin); // cout << "LS: " << LS << endl;
    fread_check(snpID, 1, LS, fin); snpID[LS] = '\0'; cout << "first snpID: " << snpID << endl;
    //fseek_check(fin, LS, SEEK_CUR); // skip SNP id
    ushort LR; fread_check(&LR, 2, 1, fin); // cout << "LR: " << LR << endl;
    fread_check(rsID, 1, LR, fin); rsID[LR] = '\0'; cout << "first rsID: " << rsID << endl;
	...
	if (b != 0) { // haven't already read snpID and rsID
	  fread_check(&LS, 2, 1, fin);
	  fread_check(snpID, 1, LS, fin); snpID[LS] = '\0';
	  fread_check(&LR, 2, 1, fin);
	  fread_check(rsID, 1, LR, fin); rsID[LR] = '\0';
	}
	...
	if (b == 0) {
	  ...
	  cout << "first variant's CHR POS A B: " << chrStr << " " << bp << " "
	       << allele1 << " " << allele0 << endl;
	  ...
	  if (Pmin != 1U && Pmin != 2U) {
	    cerr << "ERROR: " << snpName << " has minimum ploidy = " << Pmin << " (not 1 or 2)" << endl;
	    exit(1);
	  }
	  if (Pmin == 1U && chrom <= Nautosomes) {
	    cerr << "ERROR: " << snpName << " has minimum ploidy 1; autosomal ploidy must be 2" << endl;
	    exit(1);
	  }
	...
	uint len = LC + LA + LB + 20;
	if (len > variantLen) {
	  free(variant);
	  variantLen = 2*len;
	  variant = (char *) malloc(variantLen);
	}
	sprintf(variant, "%s:%d:%s:%s", chrStr, bp, allele1, allele0);
	...
	if ((b+1 == maxBgenVariantsToScan // also be sure to scan past PAR on chrX: see below
	     && (LMM::SnpData::chrStrToInt(chrStr, Nautosomes)<=Nautosomes || bp>3000000))
	    || b+1 == Mbgen) {
	  cerr << "ERROR: Scanned " << (b+1) << " variants without finding one to test" << endl;
	  cerr << "       last scanned variant's CHR POS A B: " << chrStr << " " << bp << " "
	       << allele1 << " " << allele0 << endl;
	  cerr << "       If first variant to test in this file occurs later, increase --maxBgenVariantsToScan" << endl;
	  exit(1);
	}
      }
